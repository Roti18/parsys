...
<header
	class="h-16 flex items-center justify-between px-4 sm:px-8 sticky top-0 z-10 bg-slate-50/80 dark:bg-[#141414]/80 backdrop-blur-xl"
>
	...
	<div class="flex items-center gap-2 sm:gap-4">
		...
		{#if user}
			<div
				class="flex items-center gap-3 pl-3 sm:pl-4 border-l border-slate-200 dark:border-white/8"
			>
				...
				<form action="/logout" method="POST">
					<button
						type="submit"
						class="text-slate-400 hover:text-rose-500 dark:hover:text-rose-400 p-1.5 rounded-md transition-colors"
						title="Logout"
					>
