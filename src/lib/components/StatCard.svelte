...
<div class="rounded-2xl p-5 relative overflow-hidden transition-colors {highlight ? 'bg-[#32ade6]/10 text-white' : 'bg-white dark:bg-white/[0.03] text-slate-900 dark:text-zinc-100'}">
  <div class="flex items-center justify-between">
    <div class="min-w-0">
      ...
      <h3 class="text-2xl font-bold truncate {highlight ? 'text-[#32ade6]' : 'text-slate-800 dark:text-zinc-100'}">{value}</h3>
    ...
    <div class="w-10 h-10 flex items-center justify-center shrink-0 {highlight ? 'text-[#32ade6]' : 'text-slate-400 dark:text-zinc-600'}">
